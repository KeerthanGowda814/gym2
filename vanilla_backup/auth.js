...
// Simulated Base64 encoding/decoding supporting unicode characters
...
const ApexAuth = {
  ...
  /**
   * Enforces security route guards. Redirects unauthorized users.
   */
  enforceRouteGuard(loginPagePath = 'login.html') {
    if (!this.isAuthenticated()) {
      this.logout(); // clean any expired token residues
      window.location.href = loginPagePath;
    }
  },

  /**
   * Portal page guard: Redirects logged in users away from login pages
   */
  enforcePortalGuard(dashboardPagePath = 'dashboard.html') {
    if (this.isAuthenticated()) {
